// Script to check chat_messages table for image messages
...
async function checkChatMessages() {
  try {
    console.log('🔍 Checking chat_messages table for images...');
    
    // Get all chat_messages that have image_url
    ...
    if (!messages || messages.length === 0) {
      console.log('✅ No chat_messages with images found');
      return;
    }

    console.log(`📊 Found ${messages.length} chat_messages with images:\n`);
    ...
    // If there are messages with post-images URLs, offer to fix them
    if (postImagesCount > 0) {
      console.log('\n🔧 Found messages with incorrect URLs. Would you like to fix them?');
      console.log('Run: node scripts/fix-chat-message-urls.js');
    }
